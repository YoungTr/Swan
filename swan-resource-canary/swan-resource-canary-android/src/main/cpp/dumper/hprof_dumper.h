...
#ifndef SWAN_HPROF_DUMPER_H
#define SWAN_HPROF_DUMPER_H
...
class HprofDumper {
public:
    static HprofDumper &getInstance();
    void initialize();
...
private:
    ...
    DISALLOW_COPY_AND_ASSIGN(HprofDumper);

    bool init_done;
    int android_api;

    // ScopedSuspendAll instance placeholder
    std::unique_ptr<char[]> ssa_instance;
    // ScopedGCCriticalSection instance placeholder
    std::unique_ptr<char[]> sgc_instance;
    ...
    void (*ssa_constructor_fnc)(void *handle, const char *cause, bool long_suspend);

    // art::ScopedSuspendAll::~ScopedSuspendAll()
    void (*ssa_destructor_fnc)(void *handle);

    // art::gc::ScopedGCCriticalSection::ScopedGCCriticalSection()
    void (*sgc_constructor_fnc)(void *handle, void *self, GcCause cause, CollectorType collector_type);

    // art::gc::ScopedGCCriticalSection::~ScopedGCCriticalSection()
    void (*sgc_destructor_fnc)(void *handle);

    // art::Locks::mutator_lock_
    void **mutator_lock_ptr;

    // art::ReaderWriterMutex::ExclusiveLock
    void (*exclusive_lock_fnc)(void *, void *self);

    // art::ReaderWriterMutex::ExclusiveUnlock
    void (*exclusive_unlock_fnc)(void *, void *self);
};

#endif //SWAN_HPROF_DUMPER_H
